...
var nowDate = dateObj.getDate();       // 当前几号
var nowMonth = dateObj.getMonth()+1;   // 当前月
var nowYear = dateObj.getFullYear();   // 当前年
function  init(){
	...
	$(#closeIcon).hover(function(){
		closeIcon.src = @lib + "/ico_close_P.png";
	},function(){
		closeIcon.src = @lib + "/ico_close_popup.png";
	})
	...
	_self.find(".nowTime").click(function(){
		...
		var nowHour = changeDetailTime.getHours();         //当前小时
		var nowMin = changeDetailTime.getMinutes();        //当前分钟
		var nowSec = changeDetailTime.getSeconds();        //当前秒
		_self.find(".top-change-month").html(nowMonth);
		_self.find(".top-change-year").html(nowYear);
